 ...
 */
@TargetApi(21) public class BeaconTransmitter {
  ...
  // possible
  @Deprecated public static final int NOT_SUPPORTED_MULTIPLE_ADVERTISEMENTS = 3;
   ...
   */
  public BeaconTransmitter(Context context, BeaconParser parser, String btName) {
    mBeaconParser = parser;
    BluetoothManager bluetoothManager =
        (BluetoothManager) context.getSystemService(Context.BLUETOOTH_SERVICE);
    if (bluetoothManager != null) {
      mBluetoothAdapter = bluetoothManager.getAdapter();
      if (!btName.equals("")) mBluetoothAdapter.setName(btName);
  ...
  public void startAdvertising() {
    if (mBeacon == null) {
      throw new NullPointerException(
          "Beacon cannot be null.  Set beacon before starting advertising");
    }
    ...
    if (mBeaconParser == null) {
      throw new NullPointerException(
          "You must supply a BeaconParser instance to BeaconTransmitter.");
    }
    ...
    LogManager.d(TAG,
        "Starting advertising with ID1: %s ID2: %s ID3: %s and data: %s of size " + "%s",
        mBeacon.getId1(), mBeacon.getIdentifiers().size() > 1 ? mBeacon.getId2() : "",
    ...
    try {
      ...
      if (serviceUuid > 0) {
        byte[] serviceUuidBytes = new byte[] {
            (byte) (serviceUuid & 0xff), (byte) ((serviceUuid >> 8) & 0xff)
        };
  ...
  public static int checkTransmissionSupported(Context context) {
    ...
    if (android.os.Build.VERSION.SDK_INT < 21) {
    ...
    } else if (!context.getApplicationContext()
        .getPackageManager()
      ...
      try {
        ...
        if (((BluetoothManager) context.getSystemService(Context.BLUETOOTH_SERVICE)).getAdapter()
